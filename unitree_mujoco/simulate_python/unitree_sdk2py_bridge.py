...
class UnitreeSdk2Bridge:
    ...
    def __init__(self, mj_model, mj_data):
        ...
        self.have_motor_sensors = (
            self.mj_model.nsensordata >= self.dim_motor_sensor
        )
        ...
        # Motor sensors (if present) occupy the first 3*nu entries.
        sensor_scan_start = (
            self.dim_motor_sensor if self.have_motor_sensors else 0
        )
        for i in range(sensor_scan_start, self.mj_model.nsensor):
    ...
    def LowCmdHandler(self, msg: LowCmd_):
        if self.mj_data != None and self.have_motor_sensors:
            n_cmd = min(self.num_motor, len(msg.motor_cmd))
    ...
    def PublishLowState(self):
        if self.mj_data != None:
            if self.have_motor_sensors:
                n_pub = min(self.num_motor, len(self.low_state.motor_state))
    ...
    def PublishHighState(self):
        ...
        if self.mj_data != None and self.have_frame_sensor_:
